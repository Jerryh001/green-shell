...
from ..GWTpayload import GWTPayload, respToJson
...
class ManagerBot:
    _account_url = "https://kekeke.cc/com.liquable.hiroba.gwt.server.GWTHandler/accountService"
    _anonymous_url = "https://kekeke.cc/com.liquable.hiroba.gwt.server.GWTHandler/anonymousService"
    ...
    def __init__(self, channelName: str = "綠盾防禦系統"):
        self.channelName = channelName
        self.agentUser = User("綠盾管理員#Bot", anchorUsername="__BOT__")
        self._log: logging.Logger = logging.getLogger((f"{__name__}@{self.channelName}"))
        ...
        self.redisPerfix = f"kekeke::bot::channel::{self.channelName}::"
    ...
    async def initial(self):
        await self.setUpConnector()
        await self.subscribe()
        await asyncio.gather(self.updateFlags(True), self.initMessages(), self.updateUsers(True), self.showLogo())
        self.connectEvents = self.loop.create_task(asyncio.wait({self.listen(), self.keepAlive()}))

    async def setUpConnector(self):
        while True:
            try:
                self._session: aiohttp.ClientSession = aiohttp.ClientSession(cookie_jar=aiohttp.CookieJar(unsafe=True))
                self.ws: aiohttp.ClientWebSocketResponse = await self._session.ws_connect(url=r"wss://ws.kekeke.cc/com.liquable.hiroba.websocket", heartbeat=120)
                break
    ...
    async def reConnect(self):
        await self.Close(stop=False)
        if not self.closed:
            self.loop.create_task(self.initial())
    ...
    async def updateKerma(self):
        ...
        if self.isAnchor:
            ...
            j = await self.post(payload=_payload.string, url=_account_url)
        else:
            ...
            j = await self.post(payload=_payload.string, url=_anonymous_url)
    ...
    def getGUID(self) -> str or None:
        return redis.get(f"{self.redisPerfix}botGUID")
    ...
    async def subscribe(self):
        ...
        _payload.AddPara("com.liquable.hiroba.gwt.client.square.StartSquareRequest/2186526774", [self.GUID if self.GUID else None, None, "com.liquable.gwt.transport.client.Destination/2061503238", f"/topic/{self.channelName}"])
        while True:
        ...
        data = data[-3]
        if not self.GUID:
            self.GUID = data[-5] if self.isAnchor else data[1]
        self.agentUser.ID = data[-1]
        await self.ws.send_str(f'CONNECT\nlogin:{json.dumps({"accessToken": data[-4] if self.isAnchor else data[2], "nickname": self.agentUser.nickname})}')
        await self.ws.send_str(f'SUBSCRIBE\ndestination:/topic/{self.channelName}')
        self._log.info(f"WebScoket已登入到{self.channelName}")

    async def initMessages(self):
        ...
        _payload.AddPara("com.liquable.gwt.transport.client.Destination/2061503238", [f"/topic/{self.channelName}"])
    ...
    async def listen(self):
        while not self.ws.closed:
            ...
            if publisher == "CLIENT_TRANSPORT":
                self.loop.create_task(self.on_message(m))
            elif publisher == "SERVER":
                if m.mtype == Message.MessageType.population:
                    self.loop.create_task(self.updateUsers())
                elif m.mtype == Message.MessageType.vote:
                    if m.payload["votingState"] == "CREATE":
                        if m.payload["title"] == "__i18n_voteForbidTitle":
                            self.loop.create_task(self.vote(m.payload["votingId"], "__i18n_forbid"))
                        elif m.payload["title"] == "__i18n_voteBurnTitle":
                            self.loop.create_task(self.vote(m.payload["votingId"], "__i18n_burn"))
                        elif m.payload["title"] == "__i18n_voteMinKermaTitle":
                            self.loop.create_task(self.vote(m.payload["votingId"], "__i18n_agree"))
                    elif m.payload["votingState"] == "COMPLETE":
                        if m.payload["title"] == "__i18n_voteForbidTitle":
                            self.loop.create_task(self.banCommit(m.payload["votingId"]))
                        elif m.payload["title"] == "__i18n_voteBurnTitle":
                            self.loop.create_task(self.burnCommit(m.payload["votingId"]))
                elif m.mtype == Message.MessageType.system:
                    self._log.warn(f"天之聲發言：{msg_list[3]}")
                    self.loop.create_task(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=f'【天之聲】{m.payload["message"]}', metionUsers=list(self.users)), showID=False)
                elif m.mtype == Message.MessageType.euro:
                    self.loop.create_task(self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=f'{m.payload["sucker"]["nickname"]} 吸了大家的歐氣', metionUsers=list(self.users)), showID=False))
        ...
        for i in range(3):
            async with self._session.post(url=url, data=payload, headers=header) as r:
                if r.status != 200:
                    ...
                    return await r.json(loads=respToJson)
        return None

    async def updateUsername(self):
        newname = self.agentUser.nickname + "".join(self.flags)
        await self.rename(self.agentUser, newname)
    ...
    async def updateUsers(self, initial=False) -> typing.Set[User]:
        ...
        if j:
            ...
            if flag.talk in self.flags and not initial:
                for user in joined:
    ...
    async def on_message(self, message: Message):
        ...
        if not self.isBot(message.user):
            ...
            for key in redis.smembers(f"{self.redisPerfix}reactionkeywords"):
                if re.search(key, message.content):
                    await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=redis.srandmember(f"{self.redisPerfix}reactionkeywords::{key}"), metionUsers=[message.user]), showID=False)
            if message.content[0:len(self.commendPrefix)] == self.commendPrefix:
                ...
                if args:
                    if (args[0] in command.commands):
                        self.loop.create_task(command.commands[args[0]](self, message, *(args[1:])))
    ...
    async def say(self, content: str):
        message = Message(Message.MessageType.chat, content=content, user=self.agentUser)
        await self.sendMessage(message, showID=False)

    def isBot(self, user: User):
        return re.search("#Bot$", user.nickname)

    async def anonSend(self, message: discord.Message):
        user = copy.deepcopy(self.agentUser)
        ...
        if kid:
            ...
            user.nickname = f"Bot@{message.author.display_name}"
    ...
    async def sendTextImage(self, text: str, user: User = None):
        ...
        img.save(filepath)
        text = await self.postImage(filepath)
        await self.sendMessage(Message(mtype=Message.MessageType.chat, user=user if user else self.agentUser, content=text["url"]), showID=False)
        try:
            os.remove(filepath)
        except Exception as e:
            self._log.error("刪除檔案失敗")
...
# ###########################################commands#######################################
    ...
    async def whois(self, message: Message, *args):
        if (len(message.metionUsers) == 1):
            ...
            if level == "bot":
                ...
                result += "未知使用者"
            await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=result, metionUsers=[message.metionUsers[0], message.user]), showID=False)
    ...
    async def cls(self, message: Message, *args):
        def isValid(m: Message) -> bool:
            return not self.isBot(m.user) and m.content[0:len(self.commendPrefix)] != self.commendPrefix
    ...
    async def ban(self, message: Message, *args):
        ...
        if target.ID in redis.sunion(f"{self.redisGlobalPerfix}auth", f"{self.redisPerfix}auth", f"{self.redisPerfix}members"):
            await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=f"❌使用者{target}具有成員以上身分，無法執行", metionUsers=[message.user]), showID=False)
            return
        ...
        _payload.AddPara("com.liquable.gwt.transport.client.Destination/2061503238", [f"/topic/{self.channelName}"])
        _payload.AddPara("com.liquable.hiroba.gwt.client.chatter.ChatterView/4285079082", ["com.liquable.hiroba.gwt.client.square.ColorSource/2591568017", None, self.agentUser.ID, self.agentUser.nickname, self.agentUser.ID])
    ...
    async def bind(self, message: Message, *args):
        if len(args) >= 1:
            try:
                ...
                await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content="✔️已綁定到Discord", metionUsers=[message.user]), showID=False)
            except ValueError:
                await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content="❌參數錯誤", metionUsers=[message.user]), showID=False)
    ...
    async def clear(self, message: Message, *args):
        if len(args) >= 2 and args[0] == self.channelName:
    ...
    async def zawarudo(self, message: Message, *args):
        if len(args) >= 1 and args[0] == self.channelName:
    ...
    async def save(self, message: Message, *args):
        ...
        name_safe = html.escape(self.channelName)
    ...
    async def command_load(self, message: Message, *args):
        if args[0] != self.channelName or len(args) < 2:
            return
        name_safe = html.escape(self.channelName)
    ...
    async def resetMessages(self, vaildRule):
        ...
        await self.sendMessage(Message(mtype=Message.MessageType.chat, user=self.agentUser, content=f"###DISABLE#BOT#RECORD#{len(validmessages)+len(medias)}###"), showID=False)
        for media in medias:
            user = copy.deepcopy(self.agentUser)
            user.ID = media.user.ID
        ...
        if self.timeout:
            ...
            try:
                ...
                pass
            self.timeout = self.loop.create_task(self.SelfTimeout())
    ...
    async def burnCommit(self, voteid: str):
